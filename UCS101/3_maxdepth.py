...
# Crawl the x deep
def crawl_web_u(seed, max_depth):
    tocrawl = [seed]
    crawled = []
    count = 0
    while tocrawl and max_depth >= count:
        page = tocrawl.pop()
...
def crawl_web(seed, max_depth):
    ...
    while count <= max_depth:
        ...
        while tocrawl:
            page = tocrawl.pop()
            if page not in crawled:
                union(tocrawl2, get_all_links(get_page(page)))
                crawled.append(page)
        union(tocrawl, tocrawl2)
        count += 1
    return crawled

#def crawl_web(seed, max_depth):
#    cycle = 0                                #Set iteration tally to zero
#    tocrawl = [seed]                     #tocrawl - set to seed page
#    crawled = []                         #checked pages starts at 0
#    while cycle <= max_depth:                #until iterations tally hits maximum
#        tocrawl2 = []                         #make a list of 'next depth' pages
#        while tocrawl:                       #while pages are left to be checked
#            page = tocrawl.pop()             #extract last of pages to be checked
#            if page not in crawled:          #if it's not been done already
#                union(tocrawl2, get_all_links(get_page(page)))           #add all its lnks to 'next depth' list
#                crawled.append(page)         #and mark it as checked
#        union(tocrawl, tocrawl2)                   #mark the next depth list as needing to be checked
#        cycle += 1                     #add one to the tally count
#    return crawled
...
#print(crawl_web_u("http://top.contributors/velak.html", 2))

print(crawl_web("http://top.contributors/velak.html", 2))
#['http://top.contributors/charlzz.html',
# 'http://top.contributors/elssar.html', <<- Missing
# 'http://top.contributors/forbiddenvoid.html',
# 'http://top.contributors/graemeblake.html',
# 'http://top.contributors/jesyspa.html',
# 'http://top.contributors/johang.html',
# 'http://top.contributors/kilaws.html', <<- Missing
