...
# recruited students. Define a procedure,
# hexes_to_udaciousness(n, spread,# target), that takes three inputs:
# n = the starting number of Udacians,
# spread = the spread rate (how many new friends each Udacian convinces to join each hexamester),
# target = the target number,
# and outputs the number of hexamesters needed to reach (or exceed) the target.

# For credit, your procedure must not use: while, for, or import math.

def hexes_to_udaciousness(n, spread, target, first_run=True):
    hexmester_total = n + (n * spread)
    if n > target:
        return 0 if first_run else 1
    if hexmester_total >= target:
        return 1
    return 1 + hexes_to_udaciousness(hexmester_total, spread, target - hexmester_total, False)
